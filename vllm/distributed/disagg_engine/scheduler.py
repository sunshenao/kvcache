...
from vllm.config import CacheConfig, KVTransferConfig, LoRAConfig, SchedulerConfig
...
class Scheduler:
    ...
    def __init__(
        ...
        list_lock: dict[str,threading.Condition] = None,
    ) -> None:
    ...
    def has_unfinished_seqs(self) -> bool:
        return (len(self.waiting) != 0 or len(self.running) != 0
                or len(self.swapped) != 0) or len(self.transfered)
    ...
    def get_num_unfinished_seq_groups(self) -> int:
        return len(self.waiting) + len(self.running) + len(self.swapped) + len(self.transfered)
    ...
    def _schedule_disagg(self,is_prefill):
        ...
        # If any requests are swapped, prioritized swapped requests.
        if not self.swapped and is_prefill:
            prefills = self._schedule_prefills(budget,
                                               curr_loras,
                                               enable_chunking=False)
        ...
        # only contains decode requests, not chunked prefills.
        if not is_prefill:
            with self.list_lock['transfered']:
            ...
            if (len(running_scheduled.preempted) +
                ...
                swapped_in = \
                    self._schedule_swapped(budget, curr_loras)
            if len(running_scheduled.decode_seq_groups) + len(swapped_in.decode_seq_groups) == 0:
                return None
        ...
        # Update waiting requests.
        if not is_prefill and len(running_scheduled.preempted) != 0:
            self.waiting.extendleft(running_scheduled.preempted)
            
        # Update new running requests.
        if len(prefills.seq_groups) > 0:
            with self.list_lock['transfered']:
                self.transfered.extend([s.seq_group for s in prefills.seq_groups])
        ...
        assert len(running_scheduled.prefill_seq_groups) == 0
        assert len(swapped_in.prefill_seq_groups) == 0
        # print(is_prefill,self.is_decode(),len(self.waiting),len(self.transfered),len(self.running),'-'*20)
        # Merge lists
        num_prefill_groups = len(prefills.seq_groups)
        if num_prefill_groups > 0:
            scheduled_seq_groups = prefills.seq_groups
        ...
        return SchedulerOutputs(
            ...
            ignored_seq_groups=ignored_seq_groups,
            num_lookahead_slots=running_scheduled.num_lookahead_slots,
            running_queue_size=len(self.running) if not is_prefill else len(self.transfered),
            preempted=preempted,
        )
    ...
    def _schedule_transfer(self):
        ...
        while transfered_queue and num < length:
            ...
            # If the sequence group cannot be allocated, stop.
            # can_allocate = self.block_manager.can_allocate(
            #     seq_groups, num_lookahead_slots=0)

            is_decode = seq_groups.is_decode and not seq_groups.is_prefill()
            if is_decode :
                # for seq in seq_groups.get_seqs(status=SequenceStatus.WAITING):
                #     seq.status = SequenceStatus.TRANSFERED
                self.running.extend([seq_groups])
                transfered_queue.popleft()
            else:
                transfered_queue.rotate(-1)

    def _schedule(self,is_prefill=False) -> SchedulerOutputs:
        """Schedule queued requests."""
        if self.scheduler_config.chunked_prefill_enabled:
            return self._schedule_chunked_prefill()
        elif self.is_decode():
            return self._schedule_disagg(is_prefill)
    ...
    def schedule(
        ...
        scheduler_outputs: SchedulerOutputs = self._schedule(is_prefill)
