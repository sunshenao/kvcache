...
from vllm.distributed.disagg_engine.llm_engine import LLMEngine
...
class MQLLMEngine:
    ...
    def __init__(self,
        ...
        self.prefill_con = threading.Condition() 
        self.decode_con = threading.Condition() 
        self.prefill_thread = None
        self.decode_thread = None

        self.temp = False
        self.instance: str = "prefill" if self.kv_config and self.kv_config.is_kv_producer \
                            else "decode"
        self.count = 0
    ...
    def switch(self,instance):
        if self.count > 0:
            return 
        self.count += 1
        assert not self.kv_config is None
        print(instance,'.........................')
        if self.instance == 'decode':
            self.kv_config.kv_role = "kv_producer"
            if self.prefill_thread:
                self.prefill_thread.shutdown()
                self.prefill_thread = None
            self.instance = "prefill"


        elif self.instance == 'prefill':
            self.kv_config.kv_role = "kv_consumer"
    ...
    def prefill(self,is_prefill=True):
        while True:

            with self.prefill_con:
                if not any(scheduler.have_seq_prefill() != 0 for scheduler in self.engine.scheduler):
                    self.prefill_con.wait()
                    
            self.engine_step(is_prefill)

            if self.kv_config.is_kv_producer:
                self.prefill_thread = None
                return 
    ...
    def run_engine_loop(self):
        ...
        while True:
            ...
            with self.prefill_con:
                if any(scheduler.have_seq_prefill() != 0 for scheduler in self.engine.scheduler):
                    self.prefill_con.notify_all()
    ...
    def decode(self):
        while True:
        #     with self.decode_con:
        #         if not any(scheduler.have_seq_decode() != 0 for scheduler in self.engine.scheduler):
        #             with self.prefill_con:
        #                 self.prefill_con.notify_all()
        #             self.decode_con.wait()
    ...
    def engine_step(self,is_prefill=False) -> List[RequestOutput]:
        """Engine step wrapper with error handling."""
        try:
            return self.engine.step(is_prefill)
        except SystemExit:
            raise
    ...
    def handle_new_input(self):
        ...
        try:
            while self.input_socket.poll(timeout=0) != 0:
                ...
                # if "switch" in request.equest_id:
                #     if self.instance in request.request_id:
                #         return 
                    
                #     if self.engine.has_unfinished_requests():
                #         logger.debug("存在尚未完成的数据，不允许进行转化实例")
                #         return 
                #     self.switch(request.request_id)
                #     return
